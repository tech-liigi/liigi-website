            ...
            {home?.page_sections?.map(section=>(
                <>
                    ...
                    <section style={{
                        ...
                        objectFit:"cover",
                    }} id="banner" key={section.id}  className={`sm:p-0 md:px-5 md:py-5 lg:px-5 lg:py-5 xl:px-5 xl:py-5 w-full object-cover h-full`}>
                        <div className="grid  sm:grid-cols-1 md:grid-cols-3 xl:grid-cols-4 lg:grid-cols-4 gap-4 h-full">
                            ...
                            <div className="col-span-2 z-0 sm:border-0 sm:rounded-none sm:w-full bg-white rounded-[4px] border-[1px] border-black group h-full">
                                <div className='flex flex-col items-start px-5 sm:py-0 md:py-10 lg:py-10 xl:py-10 sm:h-full md:h-[50%] lg:h-[50%] xl:h-[50%] '>
                                    <h6 className={`text-sm font-semibold py-2 ${rowdies.className} text-primary`}>{banner.items[0].category.title}</h6>
                                    {/*theme color here*/}
                                    <Link href={createSlug(banner.items[0].category.title, banner.items[0].slug.current,  banner.items[0]._type)}>
                                    <h2 className={`group-hover:underline xl:text-5xl md:text-4xl sm:text-3xl lg:text-5xl py-5 font-bold ${kanit_bold.className}`}>{banner.items[0].title}</h2>
                                    </Link>
                                        <p className={`${montserrat.className} py-3 text-sm`}>{banner.items[0].short_description}</p>
                                    <Link href={createSlug(banner.items[0].category.title, banner.items[0].slug.current,banner.items[0]._type)}>
                                ...
                                <div className='w-full relative sm:mt-5 md:mt-0 xl:mt-0 lg:mt-0 sm:h-full md:h-[50%] lg:h-[50%] xl:h-[50%] overflow-hidden cursor-pointer'>
                                    <div
                                        style={{
                                            backgroundImage: banner.items[0]._type === "video" ? `url("${banner.items[0]?.video?.preview && urlForImage(banner.items[0]?.video?.preview)?.url()}")` : `url("${banner.items[0].preview && urlForImage(banner.items[0].preview)?.url()}")`,
                                        }}
                                        className="object-cover block h-full w-full  bg-no-repeat bg-center group-hover:scale-110 duration-300"
                                    />
                                    {banner.items[0]._type == "video" && <VideoMark url={banner.items[0]?.video?.url}/>}
                                </div>
                                </Link>
                            </div>
                            <div className="sm:hidden md:grid col-span-1 grid-cols-1 xl:grid lg:grid gap-4">
                            {banner.items.slice(1, 3).map((item, i) => (
                                    <div key={item.slug.current}  className="bg-white relative rounded-[4px] border-[1px] border-black overflow-hidden max-h-[410px] min-h-[410px]">
                                        <div className='flex flex-col items-start px-4 py-5 group'>

                                            <h6 className={`text-sm font-semibold ${rowdies.className} text-primary`}>{item.category.title}</h6>
                                            <Link href={createSlug(item.category.title, item.slug.current, item._type)}>
                                            <h2 className={`group-hover:underline text-2xl cursor-pointer py-3 font-bold ${kanit.className}`}>{item.title}</h2>
                                            </Link>
                                            <Link href={createSlug(item.category.title, item.slug.current, item._type)}>
                                        ...
                                        <div   className='overflow-hidden h-[60%]'>
                                            <div    style={{
                                                backgroundImage: item._type === "video" ? `url("${item?.video?.preview && urlForImage(item?.video?.preview)?.url()}")` : `url("${item.preview && urlForImage(item.preview)?.url()}")`,
                                            }} className="bg-no-repeat w-full duration-[0.3s] h-full object-center  object-contain bg-center hover:scale-110"/>
                                            {item._type == "video" && <VideoMark url={item?.video?.url}/>}
                                        </div>
                                        </Link>
                                    </div>
                            ))}
