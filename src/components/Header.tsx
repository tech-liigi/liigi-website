...
const MainHeader = ({ paddingResize, menuItems, logo, title }: any) => {
                            ...
                            <div key={page.title}>
                                {page.visible ?
                                    <MenuItem
                                        ...
                                        link={page.link||"#"}
                                        setActive={setShowSubMenu} active={showSubMenu} item={page.title}
                                    >
                                        {!page.page_submenu ? <div></div>
                                   : <div className={"flex"}>
                                            {page?.page_submenu && page?.page_submenu?.clubs &&
                                                <div  className="grid grid-cols-5 gap-4 space-y-0 text-sm z-10">
                                                    {page.page_submenu?.clubs.map((club:any)=>(
                                                        <Link key={club.title} href={club.url || "#"}>
                                                        <div className="flex mt-4 items-center justify-center flex-col">
                                                            <Image loading={"lazy"} src={club.image && urlForImage(club?.image)?.url()} alt={"Agege_LoGo"} width={50} height={50} />
                                                            <HoveredLink href="#">{club.name}</HoveredLink>
                                                        </div>
                                                        </Link>
                                                    ))}
                                                </div>
                                            ...
                                            {page?.page_submenu && page?.page_submenu.links &&
                                                <div className={"flex flex-col justify-around items-center p-2"}>
                                                    {page.page_submenu.links.map((link:any)=>(
                                                        <HoveredLink  key={link.title} href={link.url}>{link.title}</HoveredLink>
                                                    ))}
                                                </div>
                                            }
                                        </div>
...
const MobileMainHeader = ({menuItems, logo, title}:any) => {
    ...
    const onToggle = ()=>{
        if(toggleOpened && toggle.length){
            ...
            gsap.to(`.${toggle}`, { rotation: '30', ease: 'bounce.out', duration: 0.5 });
    ...
    return (
        <section className={`flex flex-col justify-start items-center w-full max-h-screen ${open ? 'overflow-y-auto' : 'overflow-y-hidden'}`}>
            <div className="w-full flex justify-between bg-black px-10 relative">
                <Link href={"/"}>
                <Image  loading={"lazy"} className='scale-105' src={logo && urlForImage(logo)?.url()} alt={`${title}_Logo`} title={title} width={80} height={80} quality={100}/>
                </Link>
                                        ...
                                        <details onClick={()=>{
                                            ...
                                            <summary className={`flex justify-between  ${inter.className} text-lg py-3 `}>
                                               <Link href={page.link || "#"}>{page.title}</Link>
                                                <Image  className={`-rotate-90 ${page.title}`} src={"/black-arrow-down.svg"} alt={'arrow-down'} width={15} height={15}/>
                                            </summary>
                                            {page.page_submenu.clubs && <div className="max-h-[300px] overflow-y-auto overflow-x-hidden">
                                                <div className='flex flex-wrap justify-center bg-white'>
                                                    {page.page_submenu.clubs.map((club:any)=>
                                                            <a key={club.title} href={club.url || "#"} target={"_blank"} rel={"noopener"}>
                                                                <Image className="cursor-pointer duration-[0.1s] hover:scale-125" src={club.image && urlForImage(club.image)?.url()} alt={`${club.name}_Logo`} title={club.name}  width={100} height={100}/>
                                                            </a>
                                                        )}
                                                </div>
                                            </div>
                                            }
                                            {page.page_submenu.links && <div>
                                                {page.page_submenu.links.map((link:any)=>(
                                                    <Link key={link.url} href={link.url || "#"}><div className={`flex text-md underline pl-4 py-3 ${inter.className}`}>{link.title}</div></Link>
                                                    ))}
                                            </div>}
                                                </details>
                                        :  page.visible &&   <Link href={page.link || "#"}><li className={`flex text-lg py-3 ${inter.className}`}>{page.title}</li></Link>}
                            </div>
                        ))}
        ...
        <section className="flex z-10 bg-white xl:px-5 xl:pl-16 lg:px-5 lg:pl-16 md:px-0 sm:px-0">
            ...
            <div className="overflow-x-auto overflow-y-hidden flex xl:w-fit lg:w-fit xl:justify-evenly lg:justify-evenly md:justify-center sm:justify-start sm:w-full md:w-full items-center pl-4">
                {props.els?.map((el:any)=>(
                    <div key={el.href} className="px-8 sm:text-sm md:text-md lg:text-md xl:text-md cursor-pointer text-center py-5 relative font-sans-['Inter'] group">
                        <a href={`#${el.section_reference}`} className='font-medium text-md'>
                            {el.title}
                        </a>
...
const Header = (props:
    ...
    const subnav = props.menuItems?.pages?.find((p:any)=> p.id === props.pageId)?.page_subnav || [];

    return (
        // fixed here
        <header>
            <div className={"sm:block md:block lg:hidden xl:hidden"}>
                <div id="header" className='w-full fixed top-0 z-10 drop-shadow-md'><MobileMainHeader title={props.logo.title} logo={props.logo.logo} menuItems={props.menuItems}/><SubHeader els={subnav}/></div>
            </div>
            <div className={"sm:hidden md:hidden lg:block xl:block"}>
            <><TopHeader clubs={props.clubs}/><div ref={headerRef} className="w-full z-10 absolute max-h-[180px]" id="header"><MainHeader title={props.logo.title} logo={props.logo.logo} menuItems={props.menuItems} paddingResize={active}/><SubHeader els={subnav}/></div></>
            </div>
        </header>
    )
}
